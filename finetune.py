...
from utils import fit_resnet_to_face_dataset, get_params_to_update, \
...
class Resnet18Finetuner:
    ...
    def __init__(self, pretrained=True, feature_extract=True):
        ...
        self._model = fit_resnet_to_face_dataset(
    ...
    def add_final_layers(self):
        ...
        self._model.batch_norm = nn.BatchNorm2d(100, eps=1e-05, momentum=0.1,
                                                affine=True,
    ...
    def unfreeze_layer(self, layer_name: str):
        ...
        for state, param in self._model.state_dict().items():
            if layer_name in state \
                    and str(param.dtype) in dtypes:
                param.requires_grad = True

    def fit(self, dataloaders, num_epochs=25, lr=0.001, is_inception=False):
        self._model = self._model.to(self.device)
        params_to_update = get_params_to_update(self._model,
...
class Resnet18FintunerForCaseD(Resnet18Finetuner):
    ...
    def add_final_layers(self):
        ...
        self._model.batch_norm_2 = nn.BatchNorm2d(100, eps=1e-05, momentum=0.1,
                                                  affine=True,
                                                  track_running_stats=True)
        self._model.classifier = nn.Softmax()
